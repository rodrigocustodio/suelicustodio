...
const inscricaoSchema = z.object({
  ...
  whatsapp: z.string().trim().regex(/^\(\d{3}\)\s\d{3}-\d{4}$/, 'Use o formato (123) 456-7890'),
  expectativa: z.string().trim().min(10, 'Mínimo de 10 caracteres').max(2000),
  forma_pagamento: z.enum(['remitly', 'cartao'], { required_error: 'Selecione a forma de pagamento' }),
...
const GoSixPage = () => {
  ...
  const [formaPagamento, setFormaPagamento] = useState<'remitly' | 'cartao' | ''>('');
            ...
            <div className="space-y-3">
              ...
              >
                <input
                  ...
                  value="remitly"
                  checked={formaPagamento === 'remitly'}
                  onChange={() => setFormaPagamento('remitly')}
                  className="h-5 w-5 text-brand-500 focus:ring-brand-500"
                />
                <span className="font-inter text-ink-900">Remitly</span>
              </label>
              <label
                className={cn(
                  'flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors',
